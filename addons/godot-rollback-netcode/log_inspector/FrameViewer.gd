...
func replay_to_current_frame() -> void:
	...
	for frame_id in range(replay_frame, log_data.frames[replay_peer_id].size()):
		...
		if frame_data.type == LoggerRollback.FrameType.TICK and frame_data.data.get('skipped', false):
...
func _send_replay_frame_data(frame_data: LogData.FrameData) -> void:
	...
	if frame_type == LoggerRollback.FrameType.TICK:
	...
	elif frame_type == LoggerRollback.FrameType.INTERPOLATION_FRAME:
